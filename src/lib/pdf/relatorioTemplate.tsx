...
const s = StyleSheet.create({
  page: { backgroundColor: C.page, color: C.ink, fontFamily: "Helvetica", fontSize: 10, paddingBottom: 40 },
  ...
  card: { backgroundColor: C.cardBg, border: `1px solid ${C.line}`, borderRadius: 9, padding: 10, marginBottom: 6 },

  // score card
  scoreCard: { borderRadius: 12, padding: 15, flexDirection: "row", alignItems: "center" },
  scoreLeft: { width: 140, alignItems: "center", borderRight: `1px solid ${C.line}`, paddingRight: 12 },
  scoreNum: { fontSize: 42, fontFamily: "Helvetica-Bold", lineHeight: 1 },
  scoreOutOf: { fontSize: 10, color: C.inkSoft, marginTop: 2 },
  scoreBadge: { marginTop: 7, borderRadius: 20, paddingVertical: 3, paddingHorizontal: 11, fontSize: 10, fontFamily: "Helvetica-Bold" },
  ...
  barTrack: { height: 7, backgroundColor: "#E2E2EE", borderRadius: 4, marginTop: 11, overflow: "hidden" },
  barFill: { height: 7, borderRadius: 4 },

  // summary stat boxes
  summaryRow: { flexDirection: "row" },
  stat: { flex: 1, backgroundColor: C.cardBg, border: `1px solid ${C.line}`, borderRadius: 9, paddingVertical: 11, paddingHorizontal: 5, marginRight: 7, alignItems: "center" },
  statNum: { fontSize: 19, fontFamily: "Helvetica-Bold", color: C.ink },
  statLabel: { fontSize: 7.5, color: C.inkSoft, marginTop: 4, textAlign: "center", lineHeight: 1.3 },
  ...
  sevBadge: { borderRadius: 12, paddingVertical: 2, paddingHorizontal: 8, fontSize: 8, fontFamily: "Helvetica-Bold" },
  chipRow: { flexDirection: "row", flexWrap: "wrap", marginTop: 7 },
  chip: { backgroundColor: C.indigoBg, color: C.indigo, fontSize: 8.5, borderRadius: 10, paddingVertical: 2, paddingHorizontal: 8, marginRight: 5, marginBottom: 4 },

  // stealer card
  stealerCard: { backgroundColor: "#FBEDED", border: `1px solid #F4C9C9`, borderRadius: 9, padding: 11, marginBottom: 6 },
  ...
  // key/value rows
  kvRow: { flexDirection: "row", justifyContent: "space-between", paddingVertical: 5, borderBottom: `1px solid ${C.line}` },
  ...
  miniLabel: { fontSize: 8, color: C.faint, marginTop: 8, marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 },
  ...
  recDot: { width: 4, height: 4, borderRadius: 2, backgroundColor: C.indigo, marginTop: 4, marginRight: 8 },
  recText: { fontSize: 9.5, color: C.ink, flex: 1, lineHeight: 1.35 },

  // upsell
  upsell: { backgroundColor: C.indigo, borderRadius: 12, padding: 14, flexDirection: "row", alignItems: "center" },
  ...
  upsellBtn: { marginTop: 10, backgroundColor: C.white, color: C.indigo, alignSelf: "flex-start", borderRadius: 20, paddingVertical: 6, paddingHorizontal: 14, fontSize: 10, fontFamily: "Helvetica-Bold" },
  ...
  footer: { position: "absolute", bottom: 16, left: PAGE_PAD, right: PAGE_PAD, flexDirection: "row", justifyContent: "space-between" },
  ...
  return (
    <View style={[s.scoreCard, { backgroundColor: risk.bg, border: `1px solid ${risk.color}33` }]}>
      <View style={s.scoreLeft}>
        ...
        <Text style={[s.scoreBadge, { backgroundColor: risk.color, color: C.white }]}>{risk.label}</Text>
      </View>
...
function BreachCard({ b }: { b: StoredBreach }) {
  ...
  const classes = (b.dataClasses || []).map(translateDataClass).filter((v, i, a) => a.indexOf(v) === i);
  return (
    <View style={s.card} wrap={false}>
      <View style={s.bcHead}>
        ...
        <Text style={[s.sevBadge, { backgroundColor: sev.bg, color: sev.color }]}>Gravidade {sev.label}</Text>
      </View>
      {classes.length > 0 && (
  ...
  return (
    <View style={s.upsell} wrap={false}>
      <View style={s.upsellLeft}>
        <Text style={s.upsellTitle}>Proteção Total</Text>
        <Text style={s.upsellText}>
          Acompanhe continuamente sua exposição digital e seja alertado quando novos vazamentos forem identificados.
        </Text>
        <Text style={s.upsellBtn}>Assinar Proteção Total</Text>
      </View>
      {qrDataUrl ? (
        <View style={s.qrBox}>
          {/* eslint-disable-next-line jsx-a11y/alt-text */}
...
export function buildRelatorioDocument(p: RelatorioProps) {
  ...
  const stealerYears = stealer.map((b) => yearOf(b.date)).filter(Boolean).sort();
  const yearsRange =
    ...
    <Document title="Relatório de Exposição Digital — Priva" author="Priva">
      ...
      <Page size="A4" style={s.page}>
        <View style={s.band}>
          ...
          <Text style={s.hSub}>Diagnóstico personalizado da sua exposição pública na internet.</Text>
        ...
        <View style={s.content}>
          <View style={s.block}>
            ...
            <Text style={s.sectionHint}>Quanto maior, mais protegida está sua identidade digital.</Text>
            <ScoreCard score={score} risk={risk} />
          </View>

          <View style={s.block}>
            <Text style={s.sectionTitle}>Resumo em números</Text>
            <Text style={s.sectionHint}>Tudo apurado a partir dos dados reais da sua verificação.</Text>
      ...
      <Page size="A4" style={s.page}>
        ...
        <View style={s.content}>
          ...
          <Text style={s.sectionHint}>Indícios da sua exposição em bases de vazamento e na web pública.</Text>

          {stealer.length > 0 && <StealerCard count={stealer.length} years={yearsRange} creds={stealer.length} />}
          ...
          <View style={[s.block, { marginTop: 10 }]}>
            ...
            <View style={s.card}>
              <View style={s.kvRow}>
                ...
                <Text style={s.kvVal}>{cpfFound ? `${cpfEx?.count ?? 0} resultado(s)` : "Não encontrado"}</Text>
              </View>
              <View style={s.kvRow}>
                <Text style={s.kvKey}>Telefone</Text>
                <Text style={s.kvVal}>{phoneCount > 0 ? `${phoneCount} resultado(s)` : "Não encontrado"}</Text>
              </View>
            ...
            {ghRepos.length > 0 ? (
              <View style={s.card}>
                {ghRepos.slice(0, 5).map((g, i, arr) => (
                  <View key={i} style={[s.kvRow, i === arr.length - 1 ? { borderBottomWidth: 0 } : {}]}>
                    <Text style={s.kvKey}>{g.repo || "Repositório público"}</Text>
                    <Text style={[s.kvVal, { color: C.indigo, fontSize: 8.5 }]}>{g.path || ""}</Text>
                  </View>
                ))}
              </View>
            ) : (
        ...
        <View style={s.content}>
          <View style={s.block}>
            ...
            <Text style={s.sectionHint}>
              Seu Identity Score parte de uma linha de base e diminui conforme encontramos indícios de exposição.
            </Text>
            <View style={s.card}>
              ...
              <View style={[s.calcRow, { borderTop: `1px solid ${C.line}`, marginTop: 3, paddingTop: 7 }]}>
                <Text style={[s.calcKey, { fontFamily: "Helvetica-Bold", color: C.ink }]}>Score final</Text>
          ...
          <View style={s.legal}>
            <Text style={s.legalText}>
              Este relatório representa um retrato da exposição pública encontrada na data de sua geração. As informações
              apresentadas são provenientes de bases públicas e monitoramentos autorizados.
            </Text>
            <Text style={s.sourceMap}>
              <Text style={s.sourceMapStrong}>Base global de vazamentos</Text> — verificação de e-mail ·{" "}
              <Text style={s.sourceMapStrong}>Monitoramento Web Público</Text> — CPF e telefone ·{" "}
              <Text style={s.sourceMapStrong}>Repositórios de código expostos</Text> — código público
            </Text>
          </View>
        </View>
        <Footer />
