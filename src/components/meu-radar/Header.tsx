...
// A notification is worth a badge only while it is NEW. Once the panel has been
// opened, the item was seen — the badge goes and it stops nagging. Ids that were
// clicked are dropped entirely.
function loadNotifs(): Notif[] {
  ...
  try {
    ...
    return initial
      ...
      .map((n) => (seen.includes(n.id) ? { ...n, unread: false } : n));
  } catch {
    return initial;
  }
}
