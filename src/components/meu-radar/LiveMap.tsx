...
const NODES: { x: number; y: number; bright?: boolean }[] = [
  { x: 34, y: 28 },
  { x: 46, y: 22, bright: true },
  { x: 56, y: 30 },
  { x: 70, y: 33, bright: true },
  { x: 76, y: 43 },
  { x: 42, y: 50 },
  { x: 64, y: 60 },
  { x: 52, y: 64 },
...
function PulseDot({
  ...
  return (
    <span className="absolute" style={{ left: `${x}%`, top: `${y}%` }}>
      <span
        className="relative flex -translate-x-1/2 -translate-y-1/2"
...
export function LiveMap() {
  return (
    <div className="w-full">
      <div
        className="relative mx-auto w-full max-w-[306px] sm:max-w-[340px]"
      ...
      >
        <svg viewBox="0 0 1024 1024" fill="none" className="absolute inset-0 h-full w-full">
          ...
          <g clipPath="url(#brclip)">
            ...
            <g>
              <path
                d={`M${CX} ${CY} L${CX} ${CY - 640} A 640 640 0 0 1 ${CX + 601} ${CY - 219} Z`}
