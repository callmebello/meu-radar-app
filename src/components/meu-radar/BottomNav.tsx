import { Radar, KeyRound, Users, Eye, User } from "lucide-react";

export type TabId = "radar" | "credenciais" | "familia" | "darkweb" | "perfil";
...
export function BottomNav({ active, onChange }: { active: TabId; onChange: (id: TabId) => void }) {
  return (
    <nav className="sticky bottom-0 z-20 border-t border-border/60 bg-background/90 px-2 pt-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] backdrop-blur-xl">
        ...
        {tabs.map((t) => {
          ...
          return (
            <li key={t.id} className="flex-1">
              ...
              >
                <span className={`grid h-9 w-9 place-items-center rounded-xl transition ${isActive ? "bg-[var(--color-navy)] text-[var(--color-teal)]" : "text-muted-foreground"}`}>
