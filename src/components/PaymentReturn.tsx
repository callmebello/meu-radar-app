...
export function PaymentReturn() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // First touch travels with the sale. The Stripe redirect lands here with
      // checkout.stripe.com as the referrer, which is exactly where GA4's own
      // session attribution breaks — carrying it ourselves is what keeps the
      // narrative attached to the revenue.
      const attrib = attributionParams();
      track("Purchase", {
        value: purchaseValue,
        currency: "BRL",
        content_name: resolvedPlan,
        ...attrib,
      });
      gaEvent("purchase", {
        transaction_id: txId,
